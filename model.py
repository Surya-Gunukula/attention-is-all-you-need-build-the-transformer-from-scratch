...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    for special in specials:
        vocab_ids[special] = counter
        counter += 1 
    
    for sentence in sentences: 
        for word in sentence.split():
            if(word not in vocab_ids):
                vocab_ids[word] = counter
                counter += 1
...
def encode_sentence_to_ids(sentence, token_to_id, unk_token='<unk>'):
    ...
    for word in sentence.split():
        if(word in token_to_id):
            return_ids.append(token_to_id[word])
        else:
            return_ids.append(token_to_id[unk_token])
    return return_ids
...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    for output_id in ids:
        output_tokens.append(id_to_token[output_id])
...
def compute_positional_div_term(d_model):
    ...
    return torch.tensor([pow(10000, (-2*i)/d_model) for i in range(d_model // 2)])
    pass
